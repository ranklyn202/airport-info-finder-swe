# airport-info-finder-swe
A pocket flight desk that helps users find airport information, codes, locations, timezones and details before travelling.
# Airport Info Finder

A browser-based airport lookup tool built with the [AviationStack API](https://aviationstack.com). Search any airport by name, IATA code, or country, view its key details on a result card, drop a pin on a live map, compare two airports side by side, and bookmark the ones you care about.
...
- [Leaflet.js](https://leafletjs.com/) for maps, with CARTO dark-theme tiles
- [AviationStack API](https://aviationstack.com) for airport and flight data
...
1. Get a free API key at [aviationstack.com](https://aviationstack.com)
2. Open `airport-info-finder.html` in your browser
3. Paste your API key into the **AviationStack API key** field
4. Search for an airport, or try one of the popular codes (LHR, JFK, HND, SIN)
